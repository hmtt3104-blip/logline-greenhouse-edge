...
Reason: this repository is a sanitized public Logline export with verified dry-run imports/config/tests and a no-hardware synthetic integration test, but real MQTT broker behavior, Firebase replay, systemd examples, and non-private hardware/runtime integration still need validation before this repository can be treated as `READY`.
...
- [x] Add a no-hardware synthetic integration test for state-store telemetry and localhost mock HTTP dispatch.
- [ ] Add decision records for edge boundaries.
- [x] Define the smallest non-private reproducible local integration test environment.

## Experiments to run

- [x] Synthetic telemetry normalization through direct state-store updates.
...
- [x] HTTP command dispatch against a localhost mock target.
...
- [x] Repeatable pytest-based no-hardware integration path.
...
- real ESP command-dispatch behavior is tested against a non-production target after localhost mock coverage;
